...
let storedMatchId = null;
let currentView = 'setup';

// Inițializare după încărcarea paginii
document.addEventListener('DOMContentLoaded', () => {
    storedMatchId = window.localStorage.getItem('currentMatchId');
    if (storedMatchId) {
        const parsedId = parseInt(storedMatchId, 10);
        if (!Number.isNaN(parsedId)) {
            currentMatchId = parsedId;
        } else {
            window.localStorage.removeItem('currentMatchId');
            storedMatchId = null;
        }
    }
...
function startMatch(matchId) {
    ...
    .then(data => {
        if (!data?.success) {
            alert(data?.message || 'Nu am putut porni meciul. Încearcă din nou.');
            return;
        }
    ...
    .catch(() => {
        alert('Nu am putut porni meciul. Verifică conexiunea și încearcă din nou.');
    });
}
...
function loadLiveMatch() {
    ...
    if (!currentMatchId && storedMatchId) {
        const parsedId = parseInt(storedMatchId, 10);
        if (!Number.isNaN(parsedId)) {
            currentMatchId = parsedId;
        } else {
